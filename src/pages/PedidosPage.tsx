...
export default function PedidosPage() {
  ...
  return (
    <div className="h-[calc(100vh-80px)] md:h-screen w-full flex flex-col p-4 md:p-6 animate-fade-in">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-white/50 mt-1">
                Gerenciamento em tempo real do fluxo de cozinha e entrega
              </p>
            </div>
          </div>
          
          <div className="flex gap-2 bg-[#16181f] p-1 rounded-2xl w-fit">
            {tabs.map(tab => (
              <button
                key={tab.id}
             ...
             {[
               ...
               <button
                 ...
                 className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border ${
                   filtroData === btn.id 
                     ? 'bg-[#e8391a]/20 text-[#e8391a] border-[#e8391a]/30' 
                     : 'bg-[#16181f] border-[#252830] text-white/60 hover:bg-[#252830] hover:text-white'
                 }`}
               >
                 {btn.label}
               </button>
        ...
        {COLUMNS.map(col => {
          ...
          return (
            <div key={col.id} className="min-w-0 flex flex-col h-full max-h-full overflow-hidden">
               <div className={`shrink-0 flex justify-between items-center p-3 2xl:p-4 rounded-t-2xl border-t-4 ${col.color} bg-gradient-to-r ${col.gradient} border-x border-[#252830] border-b border-[#252830]/50`}>
                 <h2 className={`font-bold text-xs 2xl:text-base xl:text-sm lg:text-[11px] leading-tight ${col.textColor}`}>{col.title}</h2>
                 <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm bg-[#16181f] border ${col.color} text-white shadow-md`}>
                   {colPedidos.length}
                 </span>
               </div>
               
               <div className="flex-1 overflow-y-auto space-y-4 p-4 bg-[#16181f]/50 border-x border-b border-[#252830] rounded-b-2xl no-scrollbar relative min-h-[300px]">
                 {loading ? (
                    <div className="absolute inset-0 flex items-center justify-center text-[#e8391a] animate-pulse">
                      Carregando...
                    </div>
                 ) : colPedidos.length === 0 ? (
                    ...
                    colPedidos.map(pedido => {
                      ...
                      return (
                         <div key={pedido.id} className={`bg-gradient-to-br from-[#1c1e26] to-[#16181f] p-3 2xl:p-5 rounded-xl 2xl:rounded-2xl border ${isCancelled ? 'border-red-500/20 opacity-60' : 'border-[#252830] hover:border-[#e8391a]/30'} shadow-lg transition-all animate-fade-in group flex flex-col gap-3 2xl:gap-4 relative overflow-hidden`} >
                            ...
                            <div className="flex justify-between items-start gap-2">
                               <div className="flex flex-col gap-0.5 min-w-0">
                                  ...
                                  <h3 className="font-bold text-xs xl:text-sm 2xl:text-lg text-white leading-tight truncate" title={pedido.cliente_nome}>
                                    {pedido.cliente_nome}
                                  </h3>
                               </div>
                               ...
                               <div className="flex items-center gap-1.5 shrink-0">
                                 ...
                                 <div className={`px-1.5 py-0.5 rounded-lg bg-[#252830] flex items-center gap-1 text-[9px] 2xl:text-xs ${timeClass}`}>
                            ...
                            <div className="grid grid-cols-[1fr_auto] gap-1.5 p-2 2xl:p-3 bg-[#0c0e15]/40 rounded-xl items-center">
                               <div className="flex flex-col gap-0.5 min-w-0">
                                  {pedido.itens.slice(0, 2).map((it, i) => (
                                     <p key={i} className="text-[9px] 2xl:text-xs text-white/50 truncate">
                                        <span className="font-bold text-white/60">{it.qtd}x</span> {it.nome}
                                     </p>
                                  ))}
                                  {pedido.itens.length > 2 && (
                                     <p className="text-[8px] 2xl:text-[10px] font-bold text-[#e8391a] opacity-60">+{pedido.itens.length - 2} mais</p>
                                  )}
                               </div>
                               <div className="flex flex-col items-center gap-1 border-l border-[#252830]/10 pl-2">
                                  <span className="material-symbols-outlined text-sm text-white/30" title={pedido.canal}>{getCanalIcon(pedido.canal)}</span>
                                  <span className="text-[7px] 2xl:text-[9px] uppercase font-bold tracking-widest opacity-50 whitespace-nowrap text-white/40">{pedido.forma_pagamento}</span>
                               </div>
                            </div>

                            <div className="flex justify-between items-center mt-1">
                              <span className="text-emerald-400 font-bold text-xs xl:text-sm 2xl:text-lg">
                              ...
                              <div className="flex gap-1.5 transition-all">
                                {!isCancelled && pedido.status_kanban !== 'entregue' && (
                                  <>
                                    <button onClick={() => setCancelModalPedido(pedido)} className="w-7 h-7 2xl:w-9 2xl:h-9 flex items-center justify-center rounded-lg 2xl:rounded-xl bg-[#252830] hover:bg-red-500/10 hover:text-red-500 text-white/60 transition-colors" title="Cancelar">
                                      <span className="material-symbols-outlined text-[14px] 2xl:text-[18px]">close</span>
                                    </button>
                                    <button 
                                      onClick={() => handleAvançarStatus(pedido)}
                                      className={`h-7 2xl:h-9 px-2 2xl:px-4 flex items-center justify-center rounded-lg 2xl:rounded-xl text-[10px] 2xl:text-sm font-bold shadow-lg transition-transform hover:scale-105 active:scale-95 ${col.textColor.replace('text', 'bg').replace('500', '500/20')} ${col.textColor}`}
                                      title="Avançar"
                                    >
                                      Avançar
                                    </button>
                                ...
                                <button onClick={() => setSelectedPedido(pedido)} className="w-7 h-7 2xl:w-9 2xl:h-9 flex items-center justify-center rounded-lg 2xl:rounded-xl bg-[#252830] hover:bg-[#e8391a]/20 hover:text-[#e8391a] text-white/60 transition-colors" title="Ver Detalhes">
      ...
      {showFiltroPersonalizado && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4" onClick={() => setShowFiltroPersonalizado(false)}>
          <div className="bg-[#16181f] p-6 rounded-2xl w-full max-w-md border border-[#252830] flex flex-col gap-6 animate-fade-in shadow-2xl" onClick={e => e.stopPropagation()}>
      ...
      {cancelModalPedido && (
         <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4">
           <div className="bg-[#16181f] p-6 rounded-2xl w-full max-w-sm border border-red-500/20 flex flex-col gap-4 animate-fade-in shadow-2xl">
               ...
               <div className="flex justify-end gap-3 mt-2">
                  <button onClick={() => {setCancelModalPedido(null); setCancelReason('')}} className="px-4 py-2 rounded-xl text-white/60 hover:bg-[#252830] font-bold text-sm transition-colors">Voltar</button>
                  <button onClick={handleCancelPedido} disabled={!cancelReason.trim()} className="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm transition-colors disabled:opacity-50">Confirmar</button>
               </div>
            </div>
         </div>
      )}
      ...
      {motoboyModalPedido && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[#16181f] p-6 rounded-2xl w-full max-w-md border border-[#e8391a]/20 flex flex-col gap-4 animate-fade-in shadow-2xl">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              {motoboysDisponiveis.length === 0 ? (
                ...
                motoboysDisponiveis.map(motoboy => (
                  <button
                    ...
                    className="w-full p-4 rounded-xl border border-[#252830] hover:border-[#e8391a]/30 hover:bg-[#e8391a]/5 transition-all flex items-center gap-4 disabled:opacity-50"
                  >
                    <div className="w-12 h-12 rounded-full bg-emerald-500/10 flex items-center justify-center">
                      <span className="text-xl font-bold text-emerald-500">{motoboy.nome[0]}</span>
                    </div>
                    <div className="flex-1 text-left">
                      <div className="font-bold text-white">{motoboy.nome}</div>
      ...
      {selectedPedido && (
         <>
            ...
            <div className="fixed right-0 top-0 bottom-0 w-full md:w-[450px] bg-[#16181f] z-[100] border-l border-[#252830] shadow-2xl animate-slide-left flex flex-col">
               <div className="p-6 border-b border-[#252830] flex justify-between items-center bg-[#0c0e15]/30">
                  <div>
                    <h2 className="text-2xl font-bold text-white">Pedido #{String(selectedPedido.numero).padStart(4, '0')}</h2>
                    <div className="flex gap-2 mt-2 items-center">
                      <span className="text-[10px] tracking-widest uppercase bg-[#e8391a]/20 text-[#e8391a] px-2 py-0.5 rounded-md font-bold">{selectedPedido.canal}</span>
                      <span className="text-xs text-white/50">{format(parseISO(selectedPedido.created_at), "dd/MM 'às' HH:mm", {locale: ptBR})}</span>
                    </div>
                  </div>
                  <button onClick={() => setSelectedPedido(null)} className="w-10 h-10 rounded-full bg-[#252830] hover:bg-[#1c1e26] flex items-center justify-center text-white/60 transition-colors">
               ...
               <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
                  <section className="space-y-3">
                     ...
                     <div className="bg-[#0c0e15]/50 p-4 rounded-2xl border border-[#252830]">
                        <p className="font-bold text-lg text-white">{selectedPedido.cliente_nome}</p>
                  ...
                  {selectedPedido.endereco_entrega && (
                     <section className="space-y-3">
                        ...
                        <div className="bg-[#0c0e15]/50 p-4 rounded-2xl border border-[#252830] flex justify-between items-start gap-4">
                           <p className="text-sm text-white/80 leading-relaxed">{selectedPedido.endereco_entrega}</p>
                           <a target="_blank" rel="noreferrer" href={`https://maps.google.com/?q=${encodeURIComponent(selectedPedido.endereco_entrega)}`} className="shrink-0 w-10 h-10 flex items-center justify-center rounded-xl bg-[#e8391a]/10 text-[#e8391a] hover:bg-[#e8391a]/20 transition-colors">
                  ...
                  <section className="space-y-3">
                     ...
                     <div className="bg-[#0c0e15]/50 p-4 rounded-2xl border border-[#252830] space-y-3">
                        {selectedPedido.itens.map((it: any, i: number) => (
                           <div key={i} className="flex justify-between items-start gap-3 border-b border-[#252830]/50 pb-3 last:border-0 last:pb-0">
                              <div>
                                <p className="font-bold text-white">{it.qtd}x {it.nome}</p>
               ...
               <div className="p-6 border-t border-[#252830] bg-[#0c0e15]">
                  <div className="flex justify-between items-center mb-6">
                     <span className="text-sm text-white/50 uppercase tracking-widest font-bold">Pagamento <br/><span className="text-white">{selectedPedido.forma_pagamento}</span></span>
                     <span className="text-3xl font-bold text-emerald-400">R$ {selectedPedido.total.toFixed(2)}</span>
                  </div>
                  {selectedPedido.status_kanban !== 'cancelado' && selectedPedido.status_kanban !== 'entregue' && (
                     <button onClick={() => { handleAvançarStatus(selectedPedido) }} className="w-full h-14 bg-[#e8391a] hover:bg-[#c72f15] text-white font-bold rounded-2xl text-lg transition-transform active:scale-[0.98] shadow-lg shadow-[#e8391a]/30">
                       Avançar Pedido
                     </button>
                  )}
               </div>
